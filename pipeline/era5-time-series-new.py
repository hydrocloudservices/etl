...
def make_filename(variable, time):
    return f"{Config.E5_BUCKET_TS}/{time.strftime('%Y%m%d')}_{str.upper(variable)}_ERA5_SL_REANALYSIS.nc"
...
def get_file_pattern():
    ...
    current_filenames_in_bucket: list = [os.path.basename(filename)
                                         for filename in fs.glob(os.path.join(Config.E5_BUCKET.replace('s3://',''), '*.nc'))]
    end_date_2 = \
        max([datetime.datetime.strptime(filename.split('_')[0][0:8], '%Y%m%d').date()
             for filename in current_filenames_in_bucket]) \
        .strftime('%Y%m%d')

    delta_days = int(Config.E5_TARGET_CHUNKS_TS['time']/24)
    ...
    pattern = FilePattern(
        ...
        MergeDim(name="variable", keys=Config.E5_VARIABLES_TS_NEW)
    )
...
def create_recipe(pattern):
    ...
    recipe = XarrayZarrRecipe(
        ...
        cache_inputs=False,
        target_chunks=Config.E5_TARGET_CHUNKS_TS,
...
def post_precess_dims(recipe, end_date):
    ...
    inclusive_end_date = (datetime.datetime.strptime(end_date, '%Y%m%d') + datetime.timedelta(days=1)).strftime(
        '%Y%m%d')
    pd.date_range(Config.E5_START_DATE_TS, inclusive_end_date, freq='H', inclusive='left')\
    .to_series(name='time') \
    .to_frame() \
    .set_index('time') \
    .to_xarray() \
...
def get_next_index(years):
    ...
    try:
        ...
        # next_index = (df[df == True].index.values[0] + 1) * len(Config.E5_VARIABLES_TS_NEW)
        ...
        next_index = (df[df == True].index.values[0]) * len(Config.E5_VARIABLES_TS_NEW)
...
if __name__ == '__main__':
    ...
    with Flow("ERA5-time-series-new") as flow:
