...
def get_file_pattern():
    ...
    pattern = FilePattern(
        ...
        MergeDim(name="variable", keys=Config.E5_VARIABLES_TS)
    )
...
def get_next_index(years):
    ...
    try:
        ...
        # next_index = (df[df == True].index.values[0] + 1) * len(Config.E5_VARIABLES_TS)
        ...
        next_index = (df[df == True].index.values[0]) * len(Config.E5_VARIABLES_TS)
...
def push_data_to_bucket():
    ...
    fs.put(tmp_target.root_path, os.path.dirname(Config.E5_BUCKET_ZARR_TS), recursive=True)
