...
def get_file_pattern():
    ...
    pattern = FilePattern(
        ...
        MergeDim(name="variable", keys=Config.E5L_VARIABLES_TS_NEW)
    )
...
def get_next_index(years):
    ...
    try:
        ...
        # next_index = (df[df == True].index.values[0] + 1) * len(Config.E5L_VARIABLES_TS_NEW)
        ...
        next_index = (df[df == True].index.values[0]) * len(Config.E5L_VARIABLES_TS_NEW)
    except:
        next_index = 0
        pass
    return next_index
...
def push_data_to_bucket():
    ...
    fs.put(target.root_path, os.path.dirname(Config.E5L_BUCKET_ZARR_TS_NEW), recursive=True)
